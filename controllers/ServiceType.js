...
const model = require("../models/ServiceType");
...
exports.getOne = async (req, res, next) => {
	try {
		...
		const data = await model.findOne(service_id);
		if (!_.isEmpty(data)) {
...
exports.update = async (req, res, next) => {
	try {
		...
		const data = await model.update(service_id, req.body);
		if (!_.isEmpty(data)) {
			res.status(StatusCodes.OK).send(data[0]);
		} else {
...
exports.remove = async (req, res, next) => {
	try {
		...
		//const id = req.params.id;
		const data = await model.remove(service_id);
		if (data) {
