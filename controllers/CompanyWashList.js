...
const model = require("../models/CompanyWashList");
...
exports.getOne = async (req, res, next) => {
	try {
		const wash_list_id = req.params.wash_list_id;

		const data = await model.findOne(wash_list_id);
		if (!_.isEmpty(data)) {
...
exports.update = async (req, res, next) => {
	try {
		...
		const data = await model.update(wash_list_id, req.body);
		if (!_.isEmpty(data)) {
			res.status(StatusCodes.OK).send(data[0]);
		} else {
...
exports.remove = async (req, res, next) => {
	try {
		...
		//const id = req.params.id;
		const data = await model.remove(wash_list_id);
		if (data) {
